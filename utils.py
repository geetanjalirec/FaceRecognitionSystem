...
def load_classifier_models():
    """
    This function will load the classifier SVM model and the label classifier
    """
    le = pickle.load(open('model/LabelEncoder.sav', 'rb'))
    clf = pickle.load(open('model/SVC_classifier.sav', 'rb'))
    return le, clf

def face_identity_classifier(embedding):
    """
    Compares its received embedding with the embeddings stored in the 'embeddings' folder  by
    using the SVM classfier which is trained on all the subject embeddings.


    Args:
        embedding: (numpy array) containing the embedding that will be compared from the SVC model.

    Returns:
          result: (string) describes the most likely person that the image looks like
    """
    try:
        le, clf = load_classifier_models()
